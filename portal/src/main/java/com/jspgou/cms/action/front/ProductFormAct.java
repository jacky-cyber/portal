...
public class ProductFormAct {
	...
	public String searchDiscussPage(Long productId,Integer pageNo,
		...
		Pagination page = discussMng.getPage(productId,null,null,null,null, cpn(pageNo), 10, true);
	...
	public String insertDiscuss(Long productId, String disCon,
			HttpServletResponse response, HttpServletRequest request,ModelMap model) {
		Website web = SiteUtils.getWeb(request);
		ShopMember member = MemberThread.get();
		if (member == null) {
		...
		Discuss bean = discussMng.saveOrUpdate(productId, disCon, member.getId());
		if (bean == null) {
			ResponseUtils.renderJson(response, "sameUsually");
			return null;
		}
