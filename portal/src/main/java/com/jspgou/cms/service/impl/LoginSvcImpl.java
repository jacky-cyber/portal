...
import static com.jspgou.core.web.Constants.*;
...
public class LoginSvcImpl implements LoginSvc {
	...
	private ShopMember getMemberFromCookie(HttpServletRequest request,
		...
		if (member != null) {
			...
			session.setAttribute(request, response, SESSION_MEMBER_ID_KEY,
...
//					member.getLastname(), request, response);
	...
	private ShopMember getMemberFromSession(HttpServletRequest request,
			HttpServletResponse response, Website web) {
		Long memberId = (Long) session.getAttribute(request,
				SESSION_MEMBER_ID_KEY);
		ShopMember member = null;
		Long webId = web.getId();
		if (memberId != null) {
	...
	public ShopAdmin adminLogin(HttpServletRequest request,
		...
		session.setAttribute(request, response, SESSION_ADMIN_ID_KEY, admin
				.getId());
        session.setAttribute(request,response,CHANNELID,user.getChannelId());
        session.setAttribute(request,response,COMMERCEID,user.getCommerceId());
        session.setAttribute(request,response,CHANNELNAME,user.getChannelName());
        session.setAttribute(request,response,COMMERCENAME,user.getCommerceName());
//        String cd = (String)session.getAttribute(request,CHANNEL);
...
//        header.put("version","33");
//        header.put("timestamp","1416554058295");
//        header.put("sequence_id","14");
//        header.put("appKey","testAppKey");
//        header.put("terminal_id","12133");
//        header.put("message_type","ALL");
//        body.put("type","2");
//        body.put("user_name","test001");
//        body.put("password","666666");
...
//            re = httpStatusClient.postJson("http://dev.ifunpay.com/dhb/knocker","utf-8",jsonObject.toString());
//
//        } catch (IOException e) {
...
//        JSONArray dd = data.getJSONArray("channel");
//        HashMap<String,String> map = new HashMap<>();
//        ArrayList<HashMap<String,String>> channel = new ArrayList<>();
//        for (int i=0;i<dd.length();i++){
//            JSONObject cha = JSONObject.fromObject(dd.get(i));
//
//            map.put("id",cha.getString("channelId"));
//            map.put("channelName",cha.getString("channelName"));
//            channel.add(map);
//            map = new HashMap<>();
