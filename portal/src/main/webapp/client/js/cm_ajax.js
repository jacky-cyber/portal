...
*/
function OnEnter( field ) { if( (field.value == field.defaultValue)&&(field.readOnly==false) ) { field.value = ""; } }
...
function win_popup_close(callBackFun){
  ...
  if (win_container == null || typeof win_container == 'undefined'){
  	...
  	//$("#win_popup_show").hide("slow",function(){$("#win_popup_overlay").hide("slow",function(){$('#win_container').empty()})} );
  	$("#win_popup_show").hide("slow",function(){$('#win_container').empty()});
    if (callBackFun instanceof Function){
    	callBackFun.call();
    }
  }
...
function pop_waiting_close(){
  ...
  if (pop_waiting_frame == null || typeof pop_waiting_frame == 'undefined'){ 
    ...
    $('#pop_waiting_frame').hide("slow",function(){$('#pop_waiting_frame').empty()});
  }
}
...
function mouseMoveShowDiv(ev,url,innerHtml){
	...
	popShowDiv(mousePos,url,innerHtml)
}
/*鼠标移出对象后调用关闭弹出框*/
function mouseOutHiddenDiv(){
	//if($("#div_mouse_move_show")){
	//	$("#div_mouse_move_show").hide("slow",function(){$("#div_mouse_move_show").empty()}); 
	//}
	if(document.getElementById("div_mouse_move_show")){
		document.getElementById("div_mouse_move_show").innerHTML = "";
		document.getElementById("div_mouse_move_show").style.display='none'; 
	}
}
/*
 * 鼠标移动到某对象上弹出提示信息
 * 参数: mousePos为弹出框位置坐标{x:100,y:100}
 *      url和innerHtml二选一;
 *        url为js对象,参数url:请求url地址,param请求参数为js类,例url={url:"", param:{a:"a",b:"b"}}
 *        innerHtml为字符串
 */
function popShowDiv(mousePos,url,innerHtml){
	...
	div_show = document.getElementById("div_mouse_move_show");
	div_show.style.position = "absolute";
	div_show.style.zIndex = 10;
	div_show.style.top = (mousePos.y+19) + "px";
	...
	//$("#div_mouse_move_show").show("slow");

}
/*
 * 滚动右侧滚动条
 * y:滚动到的高度  ti:所需时间  例: scroller($('#hotel_comment').offset().top , 2000);
 */
function scroller(y,ti){
