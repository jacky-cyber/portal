 ...
 *检查电话号码或手机格式
 *如果格式不正确     返回false  正确   返回true
 * 正确格式: 3到4位区号+"-"+3到8位电话号+"-"+1到8为分机号 ; 0或没有+13或15或18+9位
 */
function isPhone(phone) {
  var reg=/(^[0-9]{3,4}-[0-9]{3,8}[-[0-9]{1,8}]?$)|(^0{0,1}1[3|5|8|4][0-9]{9}$)/;
  flag = reg.test(phone);
  if(!flag) {
    return false;
  } else {
...
function checkPostCode(postCode){
	var code = postCode.match(/[1-9]\d{5}/);
	if(code == null){
		return false;
	}
	return true;
...
/*--------------身份证----- 非法返回false, 正常返回true */
function idCard(idCardStr){
  var id_card =/(^\d{15}$)|(^\d{17}([0-9]|X|x)$)/;
  var mat =idCardStr.match(id_card);
  if(mat == null){
	 return false;
  }else{
  	 return true;
