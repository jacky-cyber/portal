function initcity(proId, cityId, city) {
    ...
    switch (document.getElementById(proId).value) {
        ...
        case "海外" : var cityOptions = new Array( "请选择地区", "", "欧洲", "欧洲", "北美", "北美", "南美", "南美", "亚洲", "亚洲", "非洲", "非洲", "大洋洲", "大洋洲"); break; default: var cityOptions = new Array("请选择地区", ""); break; }
    var cityElement = document.getElementById(cityId);
    cityElement.options.length = 0;
    for(var i = 0; i < cityOptions.length/2; i++) {
        cityElement.options[i]=new Option(cityOptions[i*2],cityOptions[i*2+1]);
        if (cityElement.options[i].value==city){
            cityElement.selectedIndex = i;
        }
    }
}
function creatprovince(proId, province){
    if(typeof proId == "undefined"){
        proId = "pro"
    }
...
//    document.orderform["pro"].options[0]=new Option("请选择省份","");
    var proElement = document.getElementById(proId);
    proElement.options[0]=new Option("请选择省份","");
    for(var i = 0; i < provinces.length; i++) {
        proElement.options[i+1]=new Option(provinces[i],provinces[i]);
        if (proElement.options[i+1].value==province){
            proElement.selectedIndex = i+1;
        }
    }
}